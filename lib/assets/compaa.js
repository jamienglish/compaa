var Compaa = {
  ...
  init: function() {
    ...
    this.storeArtifacts(function() {
      this.paintThePicture();
    }.bind(this));
  },

  storeArtifacts: function(callback) {
    var xhr = new XMLHttpRequest();
    xhr.open('GET', this.compaaHost + '/artifacts.json', true);

    xhr.onload = function() {
      this.artifacts = JSON.parse(xhr.responseText).artifacts;
      callback();
    }.bind(this);
  ...
  drawDifferenceCanvas: function() {
    ...
    [referenceImage, generatedImage].forEach(function(image) {
